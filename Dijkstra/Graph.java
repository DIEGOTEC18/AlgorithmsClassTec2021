...
class Graph
{
	...
	private List<List<Node>> adj; //Adjacency Lists
    ...
    public void dijkstra(int root){
        ...
        // Distance from the current node to the root is 0 as default:
        distances[root] = 0;

        // We need to traverse and measure all the distances in the graph:
        while(measured.size() != V) {
            ...
            // Check if the current node has been measured
            // if its measured, skip it.
            ...
            // Measure the distance of all the current node neigbours:
    ...
    private void measureNeighbours(int n) {
        ...
        for(int i = 0; i < adj.get(n).size(); i++){
            ...
            Node currentNeighbour = adj.get(n).get(i);

            if(!measured.contains(currentNeighbour.node)){

                edgeDistance = currentNeighbour.cost;
                measuredDistance = edgeDistance + distances[n];

                if(distances[currentNeighbour.node] > measuredDistance){

                    // Update the distance:
                    distances[currentNeighbour.node] = measuredDistance;

                }
                
                // Add current neighbour to the Priority Queue:
                prq.add(new Node(currentNeighbour.node, distances[currentNeighbour.node]));
...
// Base Graph Class code by Akash Humar